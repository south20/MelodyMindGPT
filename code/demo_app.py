...
import gradio as gr
import mdtex2html
from model.anyToImageVideoAudio import SouthGPTModel
import torch
import json
import tempfile
...
parser.add_argument('--model', type=str, default='southgpt')
parser.add_argument('--southgpt_ckpt_path', type=str)  # the delta parameters trained in each stages
parser.add_argument('--stage', type=int, default=3)
args = parser.parse_args()
args = vars(args)
args.update(load_config(args))
model = SouthGPTModel(**args)
delta_ckpt = torch.load(os.path.join(args['southgpt_ckpt_path'], f'pytorch_model.pt'), map_location=torch.device('cpu'))
...
def parse_reponse(model_outputs):
    ...
    for output_i, p in enumerate(model_outputs):
        if isinstance(p, str):
            ...
            text_outputs.append(_temp_output)
        elif 't_aud' in p.keys():
            #_temp_output = ''
            filename = save_audio_to_local(p.get('t_aud'))
            print(filename)
            #_temp_output = f'<Audio>{filename}</Audio> ' + _temp_output
            response += f'<audio controls playsinline><source src="./file={filename}" type="audio/wav"></audio>'
        else:
...
def parse_reponse_t_audio(model_outputs):
    response = ''
    text_outputs = []
    # 遍历model_outputs中的每一项
    for output_i, p in enumerate(model_outputs):
        if 't_aud' in p.keys():
            #_temp_output = ''
            filename = save_audio_to_local(p.get('t_aud'))
...
with gr.Blocks() as demo:
    ...
    gr.HTML("""
        <h1 align="center" style=" display: flex; flex-direction: row; justify-content: center; font-size: 25pt; "><img src='./file=southgpt.png' width="45" height="45" style="margin-right: 10px;">SOUTHM-GPT</h1>
        <h3>This is the demo page of SOUTHM-GPT, an any-to-any multimodal LLM that allows for seamless conversion and generation audio!</h3>
        <h3>The current initial version of SOUTHM-GPT, limited by the quantity of fine-tuning data and the quality of the base models, may generate some low-quality or hallucinated content. Please interpret the results with caution. We will continue to update the model to enhance its performance. Thank you for trying the demo! If you have any questions or feedback, feel free to contact us.</h3>
         """)
    ...
    with gr.Row():
        with gr.Column(scale=7, min_width=500):
            with gr.Row():
                chatbot = gr.Chatbot(label='SOUTHM-GPT Chatbot', avatar_images=((os.path.join(os.path.dirname(__file__), 'user.png')), (os.path.join(os.path.dirname(__file__), "bot.png"))),height=440)
            ...
            with gr.Tab("User Input"):
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        #audio_path = gr.Audio(type='filepath',visible=False)
        ...
        with gr.Column(scale=3, min_width=300):
            with gr.Group():
                ...
                with gr.Accordion('Image Advanced Options', open=False,visible=False):
                    ...
                    num_inference_steps_for_img = gr.Slider(10, 50, value=50, step=1, label="Number of inference steps",
                                                            interactive=True)
                with gr.Accordion('Video Advanced Options', open=False,visible=False):
                ...
                with gr.Accordion('Audio Advanced Options', open=True):
                    guidance_scale_for_aud = gr.Slider(1, 10, value=7.5, step=0.5, label="Guidance scale",
                                                       interactive=True)
                    num_inference_steps_for_aud = gr.Slider(10, 200, value=100, step=1, label="Number of inference steps",
                                                            interactive=True)
