...
from transformers import StoppingCriteria, StoppingCriteriaList, ClapModel, ClapProcessor
...
class StoppingCriteriaSub(StoppingCriteria):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor):
        ...
        for stop in self.stops:
            # 将stop转换为tensor，并将其转换为与input_ids第一个元素设备一致
            _stop = torch.tensor(stop).to(input_ids[0].device)
            # 查找input_ids中与_stop第一个元素相同的索引
            indices = torch.where(_stop[0] == input_ids)
            for i in indices:
                # 如果indices不为空
                if len(i) > 0:
                    # 如果input_ids中从i到i+len(_stop)的元素与_stop相等
...
class SouthGPTModel(nn.Module):
    """LoRA for LLaMa model"""

    def __init__(self, **args):
        super(SouthGPTModel, self).__init__()
        self.args = args
        ...
        if self.args.get('freeze_lm'):
            ...
            peft_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM,#表示模型的任务类型为语言建模（Causal Language Modeling），即给定一段文本的前缀，预测下一个词的概率分布。
        ...
        self.llama_tokenizer.pad_token = self.llama_tokenizer.eos_token#末尾靠右填充token
        self.llama_tokenizer.padding_side = "right"

        self._add_image_token()
        self._add_video_token()
        self._add_audio_token()
        # 调整预训练模型的 token embeddings 大小
        ...
        #requires_grad设置为False，即不对其进行梯度计算,这意味着在模型的反向传播过程中，这些参数不会被更新
    ...
    def Encode_MutilAudio(self, audio_paths):
        ...
        librispeech_dummy = load_dataset(audio_paths, "clean", split="validation")
    ...
    def _add_image_token(self):
        ...
        for i in range(self.args['num_gen_img_tokens']):
            ...
            #使用 llama_tokenizer 对象的 add_tokens() 方法向 tokenizer 添加额外的标记 [IMGi]，并返回添加的标记数量
            num_added_tokens = self.llama_tokenizer.add_tokens(f'[IMG{i}]')
            print(f'After adding {num_added_tokens} new tokens, tokenizer("[IMG{i}]") =',
                  self.llama_tokenizer(f'[IMG{i}]', add_special_tokens=False))
            #使用 llama_tokenizer 对象的 __call__() 方法对标记 [IMGi] 进行编码，返回编码后的标记索引
    ...
    def _add_audio_token(self):
        ...
        # Add [AUD] tokens to the vocabulary词汇表.
    ...
    def extract_multimodal_feature(self, inputs):
        ...
        # 将features中的特征拼接起来，并求和，然后添加一个维度，最后返回
    ...
    def gen_audio_ldm2(self, text,num_inference_steps=40,audio_length_in_s=5):
        return_outputs = []
        if 'music' in text.lower():
            generation_model = AudioLDM2Pipeline.from_pretrained("cvssp/audioldm2-music", torch_dtype=torch.float16).to("cuda")
            audio = generation_model(text,num_inference_steps=num_inference_steps,audio_length_in_s=audio_length_in_s).audios[0]
        else:
            generation_model = AudioLDM2Pipeline.from_pretrained("cvssp/audioldm2", torch_dtype=torch.float16).to("cuda")
            audio = generation_model(text,num_inference_steps=num_inference_steps,audio_length_in_s=audio_length_in_s).audios[0]


        return audio
    
    def process_text(self, text, all_gen_aud_idx):
        if len(all_gen_aud_idx) > 0
            music = self.gen_audio_ldm2(text,num_inference_steps=200,audio_length_in_s=9.0)
            return music
        else:
            return None
    ...
    def generate(self, inputs):
        ...
        aud_outputs_t_audio = self.process_text(inputs['prompt'],all_gen_aud_idx)
        if aud_outputs_t_audio is not None:
            return_outputs.append({'t_aud': aud_outputs_t_audio})
        ...
        if len(all_gen_img_idx) == 0 and len(all_gen_vid_idx) == 0 and len(all_gen_aud_idx) == 0:
            ...
            if len(all_gen_img_idx) > 0:
                img_outputs = ['I am an audio generation model and can not generate images']
                return_outputs.append({'img': img_outputs})
            if len(all_gen_vid_idx) > 0:
                vid_outputs = ['I am an audio generation model and can not generate video']
